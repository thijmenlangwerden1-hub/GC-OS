...
THEMES = {
    ...
    # Groen-Blauw GC gebruikt bewust exact dezelfde kleuren als Blauw-Groen.
    "Groen-Blauw GC": {
        "mode": "Dark",
        "bg_root": "#071821",
        "bg_sidebar": "#0b2430",
...
class StartupIntro(ctk.CTk):
    """Premium startup screen: clean, modern and animated."""
    def __init__(self):
        super().__init__()
        ...
        self.geometry(f"980x620+{x}+{y}")

        self.canvas=tk.Canvas(self,bg="#05070C",highlightthickness=0,bd=0)
        self.canvas.pack(fill="both",expand=True)
        # Subtle ambient rings / grid accents
        for box,col,w in [
            ((-260,-280,400,380),"#182B52",2),
            ((700,300,1220,820),"#13264A",2),
            ((720,-220,1140,200),"#102B4B",1),
        ]:
            self.canvas.create_oval(*box,outline=col,width=w)
        for yline in range(90,560,70):
            self.canvas.create_line(55,yline,925,yline,fill="#0A111D",width=1)

        card=ctk.CTkFrame(self,fg_color="#0B1019",corner_radius=36,border_width=1,border_color="#263550")
        card.place(relx=.5,rely=.5,anchor="center",relwidth=.78,relheight=.76)

        # Top accent
        ctk.CTkFrame(card,fg_color="#6D8CFF",height=4,corner_radius=4).pack(fill="x",padx=80,pady=(0,26))

        logo=ctk.CTkFrame(card,width=112,height=112,corner_radius=32,fg_color="#101A2C",
                         border_width=1,border_color="#304A78")
        logo.pack(pady=(0,14))
        logo.pack_propagate(False)
        ctk.CTkLabel(logo,text="H",text_color="#7E9AFF",
                     font=ctk.CTkFont(size=58,weight="bold")).pack(expand=True)

        ctk.CTkLabel(card,text="HUISWERK PLANNER",
                     font=ctk.CTkFont(size=30,weight="bold"),
                     text_color="#F7F9FC").pack()
        ctk.CTkLabel(card,text="Jouw schooldag. Georganiseerd.",
                     font=ctk.CTkFont(size=14),text_color="#7F8DA5").pack(pady=(5,18))

        self.phase=ctk.CTkLabel(card,text="INITIALISEREN",
                                font=ctk.CTkFont(size=10,weight="bold"),
                                text_color="#7E9AFF")
        self.phase.pack(pady=(0,3))
        self.status=ctk.CTkLabel(card,text="Applicatie voorbereiden…",
                                 font=ctk.CTkFont(size=13),text_color="#B7C0CF")
        self.status.pack(pady=(0,12))

        self.progress=ctk.CTkProgressBar(card,width=600,height=10,corner_radius=8,
        ...
        self.percent=ctk.CTkLabel(card,text="0%",
                                 ...
                                 text_color="#F7F9FC")
        self.percent.pack(pady=(8,0))

        self.dot=ctk.CTkLabel(card,text="●  ●  ●",
                              font=ctk.CTkFont(size=8),text_color="#33445F")
        self.dot.pack(pady=(12,0))
        ctk.CTkLabel(self,text="HUISWERK PLANNER  •  STARTING",
                     font=ctk.CTkFont(size=9,weight="bold"),
                     text_color="#334158").place(relx=.5,rely=.955,anchor="center")

        self.step=0
        self.messages=[
            ("INITIALISEREN","Applicatie voorbereiden…"),
            ("LADEN","Huiswerk en deadlines laden…"),
            ("CONTROLEREN","Planning controleren…"),
            ("OPTIMALISEREN","Werkruimte optimaliseren…"),
            ("KLAAR","Alles staat klaar ✓")
        ]
    ...
    def animate(self):
        ...
        self.step+=1
        total=34
        v=min(self.step/total,1)
        self.progress.set(v)
        self.percent.configure(text=f"{int(v*100)}%")
        ph,msg=self.messages[min(self.step//7,4)]
        self.phase.configure(text=ph)
        self.status.configure(text=msg)
        self.dot.configure(text=("●  ○  ○" if self.step%3==0 else "●  ●  ○" if self.step%3==1 else "○  ●  ●"))
        if self.step<total:
            self.after(58,self.animate)
        else:
            self.after(550,self.open_app)
...
class ClosingIntro(ctk.CTk):
    """Premium closing screen with a calm, polished finish."""
    def __init__(self):
        super().__init__()
        self.title("Huiswerk Planner")
        self.geometry("980x620")
        ...
        self.canvas.pack(fill="both",expand=True)
        for box,col,w in [
            ((-250,300,390,900),"#123126",2),
            ((690,-230,1210,310),"#102A22",2),
        ]:
            self.canvas.create_oval(*box,outline=col,width=w)
        for yline in range(90,560,70):
            self.canvas.create_line(55,yline,925,yline,fill="#0A1512",width=1)

        card=ctk.CTkFrame(self,fg_color="#0B1019",corner_radius=36,border_width=1,border_color="#294337")
        card.place(relx=.5,rely=.5,anchor="center",relwidth=.78,relheight=.76)
        ctk.CTkFrame(card,fg_color="#36D58A",height=4,corner_radius=4).pack(fill="x",padx=80,pady=(0,26))

        logo=ctk.CTkFrame(card,width=112,height=112,corner_radius=32,fg_color="#10271F",
                         border_width=1,border_color="#2D5A46")
        logo.pack(pady=(0,14))
        logo.pack_propagate(False)
        ctk.CTkLabel(logo,text="✓",text_color="#45E69A",
                     font=ctk.CTkFont(size=50,weight="bold")).pack(expand=True)

        ctk.CTkLabel(card,text="TOT DE VOLGENDE KEER",
                     font=ctk.CTkFont(size=29,weight="bold"),
                     text_color="#F7F9FC").pack()
        ctk.CTkLabel(card,text="Je planning is afgerond. Tijd om af te sluiten.",
                     font=ctk.CTkFont(size=14),text_color="#8FA09A").pack(pady=(6,18))

        self.phase=ctk.CTkLabel(card,text="OPSLAAN",
                                font=ctk.CTkFont(size=10,weight="bold"),
                                text_color="#45E69A")
        self.phase.pack(pady=(0,3))
        self.status=ctk.CTkLabel(card,text="Wijzigingen veilig opslaan…",
                                 font=ctk.CTkFont(size=13),text_color="#B7C0CF")
        self.status.pack(pady=(0,12))
        self.progress=ctk.CTkProgressBar(card,width=600,height=10,corner_radius=8,
                                         fg_color="#17211F",progress_color="#36D58A")
        self.progress.set(0)
        self.progress.pack()
        self.percent=ctk.CTkLabel(card,text="0%",
                                 font=ctk.CTkFont(size=12,weight="bold"),
        ...
        self.percent.pack(pady=(8,0))
        self.dot=ctk.CTkLabel(card,text="●  ●  ●",
                              font=ctk.CTkFont(size=8),text_color="#315042")
        self.dot.pack(pady=(12,0))
        ctk.CTkLabel(self,text="HUISWERK PLANNER  •  VEILIG AFSLUITEN",
                     font=ctk.CTkFont(size=9,weight="bold"),
                     text_color="#3B5148").place(relx=.5,rely=.955,anchor="center")

        self.step=0
        self.messages=[
            ("OPSLAAN","Wijzigingen veilig opslaan…"),
            ("CONTROLEREN","Laatste gegevens controleren…"),
            ("AFRONDEN","Alles netjes afronden…"),
            ("AFSLUITEN","Applicatie afsluiten…"),
            ("TOT ZIENS","Denk aan je huiswerk hè! 😉")
        ]
    ...
    def animate(self):
        ...
        self.progress.set(v)
        self.percent.configure(text=f"{int(v*100)}%")
        ph,msg=self.messages[min(self.step//6,4)]
        self.phase.configure(text=ph)
        self.status.configure(text=msg)
        self.dot.configure(text=("●  ○  ○" if self.step%3==0 else "●  ●  ○" if self.step%3==1 else "○  ●  ●"))
        if self.step<total:
...
class UpdateWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent):
        ...
        # Ambient background
        bg = tk.Canvas(self, bg="#05070C", highlightthickness=0, bd=0)
        bg.place(relwidth=1, relheight=1)
        bg.create_oval(-180,-220,350,310,outline="#16284B",width=2)
        bg.create_oval(650,430,1100,880,outline="#10233F",width=2)

        card = ctk.CTkFrame(self, fg_color="#0B1019", corner_radius=36,
                            border_width=1, border_color="#263550")
        card.pack(fill="both", expand=True, padx=24, pady=24)

        ctk.CTkFrame(card, fg_color=accent, height=4, corner_radius=4).pack(
            fill="x", padx=72, pady=(0,22))

        header = ctk.CTkFrame(card, fg_color="transparent")
        header.pack(fill="x", padx=52)
        logo = ctk.CTkFrame(header,width=68,height=68,corner_radius=20,
                            fg_color="#111B2D",border_width=1,border_color="#304A78")
        logo.pack(side="left")
        logo.pack_propagate(False)
        ctk.CTkLabel(logo,text="H",text_color=accent,
                     font=ctk.CTkFont(size=34,weight="bold")).pack(expand=True)

        titles=ctk.CTkFrame(header,fg_color="transparent")
        titles.pack(side="left",padx=(16,0),fill="y")
        ctk.CTkLabel(titles,text="UPDATE CENTER",anchor="w",
                     font=ctk.CTkFont(size=25,weight="bold"),
                     text_color="#F7F9FC").pack(anchor="w")
        ctk.CTkLabel(titles,text="Huiswerk Planner • veilig bijwerken",
                     anchor="w",font=ctk.CTkFont(size=12),
                     text_color="#7F8DA5").pack(anchor="w",pady=(2,0))

        self.status=ctk.CTkLabel(card,text="Updates zoeken...",
                                font=ctk.CTkFont(size=13,weight="bold"),
                                text_color="#C1CAD8")
        self.status.pack(pady=(18,5))

        self.version_label=ctk.CTkLabel(card,text="VERBINDEN MET UPDATE-SERVER",
                                         font=ctk.CTkFont(size=9,weight="bold"),
                                         text_color="#5F6D83")
        self.version_label.pack(pady=(0,14))

        # Phase 01
        row1=ctk.CTkFrame(card,fg_color="#0F1622",corner_radius=18,
                         border_width=1,border_color="#1E2B40")
        row1.pack(fill="x",padx=52,pady=(0,8))
        ctk.CTkLabel(row1,text="01",width=34,height=34,corner_radius=12,
                     fg_color="#17243A",text_color=accent,
                     font=ctk.CTkFont(size=11,weight="bold")).pack(side="left",padx=(14,10),pady=13)
        self.download_title=ctk.CTkLabel(row1,text="DOWNLOADEN",anchor="w",
                                         font=ctk.CTkFont(size=10,weight="bold"),
                                         text_color=accent)
        self.download_title.pack(anchor="w",pady=(10,0))
        self.download_info=ctk.CTkLabel(row1,text="Wachten...  •  0%",
                                        anchor="w",font=ctk.CTkFont(size=10),
                                        text_color="#6F7B92")
        self.download_info.pack(anchor="w",pady=(1,4))
        self.download_progress=ctk.CTkProgressBar(row1,width=570,height=9,corner_radius=8,
                                                  fg_color="#172131",progress_color=accent)
        self.download_progress.set(0)
        self.download_progress.pack(fill="x",padx=(0,14),pady=(0,13))

        # Phase 02
        row2=ctk.CTkFrame(card,fg_color="#0F1622",corner_radius=18,
                         border_width=1,border_color="#1E2B40")
        row2.pack(fill="x",padx=52,pady=(0,8))
        ctk.CTkLabel(row2,text="02",width=34,height=34,corner_radius=12,
                     fg_color="#17231F",text_color="#36D58A",
                     font=ctk.CTkFont(size=11,weight="bold")).pack(side="left",padx=(14,10),pady=13)
        self.install_title=ctk.CTkLabel(row2,text="INSTALLEREN",anchor="w",
                                        font=ctk.CTkFont(size=10,weight="bold"),
                                        text_color="#5F6D83")
        self.install_title.pack(anchor="w",pady=(10,0))
        self.install_info=ctk.CTkLabel(row2,text="Wacht op download...  •  0%",
                                       anchor="w",font=ctk.CTkFont(size=10),
                                       text_color="#6F7B92")
        self.install_info.pack(anchor="w",pady=(1,4))
        self.install_progress=ctk.CTkProgressBar(row2,width=570,height=9,corner_radius=8,
                                                 fg_color="#172131",progress_color="#36D58A")
        self.install_progress.set(0)
        self.install_progress.pack(fill="x",padx=(0,14),pady=(0,13))

        self.percent=ctk.CTkLabel(card,text="0%",
                                 font=ctk.CTkFont(size=20,weight="bold"),
                                 text_color="#F7F9FC")
        self.percent.pack(pady=(10,0))
        self.speed=ctk.CTkLabel(card,text="Gereed voor controle",
                                font=ctk.CTkFont(size=10),text_color="#59677C")
        self.speed.pack(pady=(1,0))

        self.close_button=ctk.CTkButton(card,text="Annuleren",height=38,
                                        command=self.cancel,fg_color="#171E2A",
                                        text_color="#8F9BB3",hover_color="#202A3A")
        self.close_button.pack(fill="x",padx=52,pady=(14,18))

        ctk.CTkLabel(self,text="HUISWERK PLANNER  •  UPDATE CENTER",
                     font=ctk.CTkFont(size=9,weight="bold"),
                     text_color="#354257").place(relx=.5,rely=.965,anchor="center")
...
class HuiswerkApp(ctk.CTk):
    ...
    def show_huiswerk(self):
        ...
        ctk.CTkFrame(
            top, fg_color=t["accent"], height=3, corner_radius=3
        ...
        status_strip = ctk.CTkFrame(
            ...
            corner_radius=12,
            border_width=1,
            border_color=t.get("border", t["bg_main"]),
        )
        status_strip.pack(fill="x", padx=32, pady=(0, 8))
        ctk.CTkLabel(
            status_strip,
            text="●  LIVE OVERZICHT",
        ...
        ctk.CTkLabel(
            status_strip,
            text=top_status.replace("📌 ", "").replace("▶ ", ""),
            font=("Segoe UI", 10),
            text_color=t["muted"],
        ...
        for icon, label, value, color in [("📚", "Totaal", totaal, t["text"]), ("⏳", "Openstaand", openstaand, ORANJE), ("✅", "Afgerond", afgerond, GROEN), ("🔥", "Komende 3 dagen", urgent, ROOD if urgent else t["muted"])]:
            ...
            ctk.CTkLabel(card, text=f"{icon}  {label}", font=("Segoe UI", 10, "bold"), text_color=t["muted"]).pack(anchor="w", padx=14, pady=(10, 0))
            ...
            ctk.CTkLabel(card, text=str(value), font=("Segoe UI", 24, "bold"), text_color=color).pack(anchor="w", padx=14, pady=(0, 10))
